...
- **Safety checks:** the installer handles regular files only and refuses if `config.toml`, the unit, the launcher, `main.cf` or `master.cf` is a symlink. It also refuses if a file changed between the dry run and the apply. If `configure-postfix` fails part-way, it restores the previous files itself and says so.
