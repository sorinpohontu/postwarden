...
**Status: in preparation for 1.0.0, not yet published.** Feature-complete: the policy daemon with SPF/DKIM verification, configuration tools (`check-config`, `show-config`), the systemd unit, the installer (`scripts/install.py`: inspect, dry-run, apply, rollback) and operator documentation ([Installation](docs/Installation.md), [Configuration](docs/Configuration.md), [Operations](docs/Operations.md), [Migration](docs/Migration.md)). The test matrix and release procedure (`docs/Testing.md`, `docs/Releasing.md`) are in the source repository only. An earlier version passed the automated installer and the full test matrix in enforcement mode on Debian 13 (Postfix 3.10) and Debian 12 (Postfix 3.7) test servers with real traffic, including migration from pipe-based content filters, rollback, DNS failure and alias, forwarder and BCC delivery. Since then the configuration was simplified (protected addresses per mailbox, trusted networks read from Postfix) and the installer hardened; the current version still has to repeat those server tests, plus a run of the manual installation instructions, before publication. See the [changelog](CHANGELOG.md) for what 1.0.0 contains and the [roadmap](#roadmap) for what comes next.
